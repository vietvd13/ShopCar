            ...
            >
                <b-input-group>
                    <b-input-group-prepend>
                        ...
                        <b-button 
                            variant="danger"
                            @click="handleDeleteModel(idxModelName)"
                        >
                        ...
                        <b-button 
                            variant="success"
                            @click="handleAddModel(idxModelName)"
                        >
                ...
                >
                    ...
                    <b-input-group>
                        <b-input-group-prepend>
                            ...
                            <b-button 
                                variant="danger"
                                @click="handleDeleteRating(idxModelName, idxModelDetail, idxRating)"
                            >
                                <i class="fas fa-trash" />
                            </b-button>
    ...
    methods: {
        handleAddModel(indexAdd = null) {
            ...
            if (indexAdd !== null && indexAdd >= 0) {
                this.categoryTree = this.addItemIntoIndexArr(this.categoryTree, indexAdd, newModelName);
            } else {
        ...
        handleAddDetailModel(indexModel, indexAdd = null) {
            ...
            if (indexAdd !== null & indexAdd >= 0) {
                this.categoryTree[indexModel].model_detail = this.addItemIntoIndexArr(this.categoryTree[indexModel].model_detail, indexAdd, newDetailModel);
            } else {
        ...
        handleAddRating(indexModel, indexModelDetail) {
            ...
            if (!(this.categoryTree[indexModel].model_detail[indexModelDetail].rating)) {
                this.categoryTree[indexModel].model_detail[indexModelDetail].rating = [];
            }

            (this.categoryTree[indexModel].model_detail[indexModelDetail].rating).push(newRating);
        },
        handleDeleteRating(indexModel, indexModelDetail, indexRating) {
            if (
                ...
                indexRating >= 0 &&
                indexRating <= this.categoryTree[indexModel].model_detail[indexModelDetail].rating.length
            ) {
                (this.categoryTree[indexModel].model_detail[indexModelDetail].rating).splice(indexRating, 1);
            }
        },
