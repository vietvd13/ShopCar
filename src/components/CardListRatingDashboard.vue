...
export default {
    ...
    data() {
        return {
            selected: this.$store.getters.isFilterDashboard.rating,
        }
    },
    computed: {
        seletecdStore() {
            return this.$store.getters.isFilterDashboard.rating;
        },
        actionRatingDashboard() {
            return this.$store.getters.actionRatingDashboard;
        }
    },
    watch: {
        seletecdStore(newValue, oldValue) {
            if (JSON.stringify(newValue) !== JSON.stringify(oldValue)) {
                this.selected = this.$store.getters.isFilterDashboard.rating;
            }
        },
        actionRatingDashboard() {
            this.selected = [];
